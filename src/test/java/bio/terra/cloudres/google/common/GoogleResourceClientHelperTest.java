...
public class GoogleResourceClientHelperTest {
    ...
    private static final List<TagValue> ERROR_COUNT = Arrays.asList(TagValue.create(CLIENT), TagValue.create(CloudOperation.GOOGLE_CREATE_PROJECT.name()), null);
    ...
    public void testExecuteGoogleCloudCall_success() throws Exception {
        ...
        // One cloud api count
        assertEquals(AggregationData.CountData.create(1), MetricsHelper.viewManager.getView(API_VIEW_NAME).getAggregationMap().get(API_COUNT));
        // no errors
        assertNull(MetricsHelper.viewManager.getView(ERROR_VIEW_NAME).getAggregationMap().get(ERROR_COUNT));
    }
    ...
    public void testExecuteGoogleCloudCall_withException() throws Exception {
        ...
        assertThrows(ResourceManagerException.class, () -> helper.executeGoogleCloudCall(mockCallable, CloudOperation.GOOGLE_CREATE_PROJECT));
        // One cloud api count
        assertEquals(AggregationData.CountData.create(1), MetricsHelper.viewManager.getView(API_VIEW_NAME).getAggregationMap().get(API_COUNT));
        // One cloud a errors
