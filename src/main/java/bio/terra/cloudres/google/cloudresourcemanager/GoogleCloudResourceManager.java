...
public class GoogleCloudResourceManager {
  ...
  public GoogleCloudResourceManager(Credentials credentials)
  ...
  public boolean addPolicyBindingsRaw(String projectId, Map<String, Set<String>> policiesToAdd)
      throws IOException {
    Policy existingPolicy = cloudResourceManager.projects().getIamPolicy(projectId, null).execute();

    List<Binding> bindings =
        existingPolicy.getBindings() == null ? new ArrayList<>() : existingPolicy.getBindings();
    ...
    for (Binding binding : bindings) {
      ...
      if (newMembers != null) {
        Set<String> existingMembers =
            new HashSet<>(
                binding.getMembers() == null ? Collections.emptyList() : binding.getMembers());
    ...
    if (updateRequired) {
      SetIamPolicyRequest policyRequest =
          new SetIamPolicyRequest().setPolicy(existingPolicy.setBindings(bindings));
