...
public class CreateProjectWorkflowImpl implements CreateProjectWorkflow {
  private final GoogleCloudResourceManagerActivities cloudResActivities =
      Workflow.newActivityStub(
          GoogleCloudResourceManagerActivities.class, WorkflowSupport.standardRetryOptions());

  private final GoogleCloudBillingActivities cloudBillingActivities =
      Workflow.newActivityStub(
          GoogleCloudBillingActivities.class, WorkflowSupport.standardRetryOptions());
  ...
  public void createProject(CreateProjectArguments arguments) {
    ...
    try {
      String createOperationName =
          this.cloudResActivities.createProject(arguments.projectId, arguments.parent);
      saga.addCompensation(this.cloudResActivities::deleteProject, arguments.projectId);
      CloudResourceOpPollingWorkflow projectPoller =
          Workflow.newChildWorkflowStub(CloudResourceOpPollingWorkflow.class);
      ...
      String enableServicesOperationName =
          cloudServiceUsageActivities.batchEnable(
              arguments.projectId,
              Arrays.asList(
      ...
      ServiceUsageOpPollingWorkflow servicePoller =
          Workflow.newChildWorkflowStub(ServiceUsageOpPollingWorkflow.class);
      servicePoller.pollUntilDone(enableServicesOperationName);

      cloudStorageActivities.createBucket(
          "storage-logs-" + arguments.projectId, arguments.projectId);
